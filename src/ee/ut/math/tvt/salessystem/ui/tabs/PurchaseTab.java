...
public class PurchaseTab {
				...
				try {
					...
					if (payed >= Double.parseDouble(sumField.getText())) {
						try {
							...
							Order order = new Order(Double.parseDouble(sumField
									.getText()), model
									.getCurrentPurchaseTableModel().toString(),
									model.getHistoryTableModel().getLastId());
							for (SoldItem item : model
									.getCurrentPurchaseTableModel()
									.getSoldItems()) {
	...
	private JFrame createPaymentFrame() {
		...
		double sum = 0.0;
		List<SoldItem> list = model.getCurrentPurchaseTableModel()
				.getTableRows();
		for (SoldItem i : list) {
			sum += i.getSum();
		}
			...
			public void keyPressed(KeyEvent ke) {
				if (ke.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					try {
						payed = Double.parseDouble(paymentAmountField.getText());
						if (payed >= finalSum) {
							changeAmountField.setText((double) (Math
									.round((payed - finalSum) * 100))
									/ 100
									+ "");
